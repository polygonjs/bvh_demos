...
import { requiredImports_bvh_demo_sdf } from "./requiredImports";

const loadSceneFromSceneData_bvh_demo_sdf = async function (options) {
  const {
    domElement,
    sceneData,
    onProgress,
  ...
  if (runRegister) {
    ...
    for (const node of requiredImports_bvh_demo_sdf.nodes) {
      Poly.registerNode(node, undefined, { printWarnings });
    }
    for (const operation of requiredImports_bvh_demo_sdf.operations) {
      Poly.registerOperation(operation, { printWarnings });
    }
    for (const jsFunction of requiredImports_bvh_demo_sdf.jsFunctions) {
...
export { Poly, loadSceneFromSceneData_bvh_demo_sdf };
