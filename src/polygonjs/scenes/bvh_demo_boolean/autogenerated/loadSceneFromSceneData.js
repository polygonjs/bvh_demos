...
import { requiredImports_bvh_demo_boolean } from "./requiredImports";

const loadSceneFromSceneData_bvh_demo_boolean = async function (options) {
  const {
    domElement,
    sceneData,
    onProgress,
  ...
  if (runRegister) {
    ...
    for (const node of requiredImports_bvh_demo_boolean.nodes) {
      Poly.registerNode(node, undefined, { printWarnings });
    }
    for (const operation of requiredImports_bvh_demo_boolean.operations) {
      Poly.registerOperation(operation, { printWarnings });
    }
    for (const jsFunction of requiredImports_bvh_demo_boolean.jsFunctions) {
  ...
  const sceneName = "bvh_demo_boolean";
...
export { Poly, loadSceneFromSceneData_bvh_demo_boolean };
