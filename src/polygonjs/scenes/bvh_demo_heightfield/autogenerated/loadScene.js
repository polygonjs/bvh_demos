import { loadSceneData_bvh_demo_heightfield } from "./loadSceneData.js";

export const loadScene_bvh_demo_heightfield = async function (options = {}) {
  const {
    onProgress,
    domElement,
    configureSceneData,
  ...
  const promises = [
    ...
    sceneData == null
      ? loadSceneData_bvh_demo_heightfield({ onProgress, sceneDataRoot })
      : (() => {
          return new Promise((resolve) => resolve());
        })(),
  ];
  const results = await Promise.all(promises);
  const { Poly, loadSceneFromSceneData_bvh_demo_heightfield } = results[0];
  if (sceneData == null) {
    sceneData = results[1];
  }
  ...
  const loadedData = await loadSceneFromSceneData_bvh_demo_heightfield({
    onProgress,
    sceneData,
    domElement,
    runRegister,
