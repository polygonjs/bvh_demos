...
import { requiredImports_bvh_demo_fps_controls } from "./requiredImports";

const loadSceneFromSceneData_bvh_demo_fps_controls = async function (options) {
  const {
    domElement,
    sceneData,
    onProgress,
  ...
  if (runRegister) {
    ...
    for (const node of requiredImports_bvh_demo_fps_controls.nodes) {
      Poly.registerNode(node, undefined, { printWarnings });
    }
    for (const operation of requiredImports_bvh_demo_fps_controls.operations) {
      Poly.registerOperation(operation, { printWarnings });
    }
    for (const jsFunction of requiredImports_bvh_demo_fps_controls.jsFunctions) {
  ...
  const sceneName = "bvh_demo_fps_controls";
