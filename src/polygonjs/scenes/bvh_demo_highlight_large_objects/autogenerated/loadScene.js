import { loadSceneData_bvh_demo_highlight_large_objects } from "./loadSceneData.js";

export const loadScene_bvh_demo_highlight_large_objects = async function (
  options = {}
) {
  const {
    onProgress,
  ...
  const promises = [
    ...
    sceneData == null
      ? loadSceneData_bvh_demo_highlight_large_objects({
          onProgress,
          sceneDataRoot,
        })
      : (() => {
  ...
  const { Poly, loadSceneFromSceneData_bvh_demo_highlight_large_objects } =
    results[0];
  if (sceneData == null) {
    sceneData = results[1];
  }
  ...
  const loadedData =
    await loadSceneFromSceneData_bvh_demo_highlight_large_objects({
      onProgress,
      sceneData,
      domElement,
      runRegister,
