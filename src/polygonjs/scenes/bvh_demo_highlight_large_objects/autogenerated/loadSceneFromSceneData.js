...
import { requiredImports_bvh_demo_highlight_large_objects } from "./requiredImports";

const loadSceneFromSceneData_bvh_demo_highlight_large_objects = async function (
  options
) {
  const {
    domElement,
  ...
  if (runRegister) {
    ...
    for (const node of requiredImports_bvh_demo_highlight_large_objects.nodes) {
      Poly.registerNode(node, undefined, { printWarnings });
    }
    for (const operation of requiredImports_bvh_demo_highlight_large_objects.operations) {
      Poly.registerOperation(operation, { printWarnings });
    }
    for (const jsFunction of requiredImports_bvh_demo_highlight_large_objects.jsFunctions) {
  ...
  const sceneName = "bvh_demo_highlight_large_objects";
...
export { Poly, loadSceneFromSceneData_bvh_demo_highlight_large_objects };
