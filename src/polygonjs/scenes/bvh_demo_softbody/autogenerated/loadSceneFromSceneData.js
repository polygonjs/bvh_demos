...
import { requiredImports_bvh_demo_softbody } from "./requiredImports";

const loadSceneFromSceneData_bvh_demo_softbody = async function (options) {
  const {
    domElement,
    sceneData,
    onProgress,
  ...
  if (runRegister) {
    ...
    for (const node of requiredImports_bvh_demo_softbody.nodes) {
      Poly.registerNode(node, undefined, { printWarnings });
    }
    for (const operation of requiredImports_bvh_demo_softbody.operations) {
      Poly.registerOperation(operation, { printWarnings });
    }
    for (const jsFunction of requiredImports_bvh_demo_softbody.jsFunctions) {
  ...
  const sceneName = "bvh_demo_softbody";
...
export { Poly, loadSceneFromSceneData_bvh_demo_softbody };
