...
import { requiredImports_bvh_demo_multiple_rays } from "./requiredImports";

const loadSceneFromSceneData_bvh_demo_multiple_rays = async function (options) {
  const {
    domElement,
    sceneData,
    onProgress,
  ...
  if (runRegister) {
    ...
    for (const node of requiredImports_bvh_demo_multiple_rays.nodes) {
      Poly.registerNode(node, undefined, { printWarnings });
    }
    for (const operation of requiredImports_bvh_demo_multiple_rays.operations) {
      Poly.registerOperation(operation, { printWarnings });
    }
    for (const jsFunction of requiredImports_bvh_demo_multiple_rays.jsFunctions) {
  ...
  const sceneName = "bvh_demo_multiple_rays";
...
export { Poly, loadSceneFromSceneData_bvh_demo_multiple_rays };
