...
import { requiredImports_bvh_demo_delete_inside_object } from "./requiredImports";

const loadSceneFromSceneData_bvh_demo_delete_inside_object = async function (
  options
) {
  const {
    domElement,
  ...
  if (runRegister) {
    ...
    for (const node of requiredImports_bvh_demo_delete_inside_object.nodes) {
      Poly.registerNode(node, undefined, { printWarnings });
    }
    for (const operation of requiredImports_bvh_demo_delete_inside_object.operations) {
      Poly.registerOperation(operation, { printWarnings });
    }
    for (const jsFunction of requiredImports_bvh_demo_delete_inside_object.jsFunctions) {
  ...
  const sceneName = "bvh_demo_delete_inside_object";
