import { loadSceneData_bvh_demo_lightning } from "./loadSceneData.js";

export const loadScene_bvh_demo_lightning = async function (options = {}) {
  const {
    onProgress,
    domElement,
    configureSceneData,
  ...
  const promises = [
    ...
    sceneData == null
      ? loadSceneData_bvh_demo_lightning({ onProgress, sceneDataRoot })
      : (() => {
  ...
  const { Poly, loadSceneFromSceneData_bvh_demo_lightning } = results[0];
  if (sceneData == null) {
    sceneData = results[1];
  }
  ...
  const loadedData = await loadSceneFromSceneData_bvh_demo_lightning({
    onProgress,
    sceneData,
    domElement,
    runRegister,
