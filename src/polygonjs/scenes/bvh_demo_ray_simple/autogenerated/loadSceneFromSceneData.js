...
import { requiredImports_bvh_demo_ray_simple } from "./requiredImports";

const loadSceneFromSceneData_bvh_demo_ray_simple = async function (options) {
  const {
    domElement,
    sceneData,
    onProgress,
  ...
  if (runRegister) {
    ...
    for (const node of requiredImports_bvh_demo_ray_simple.nodes) {
      Poly.registerNode(node, undefined, { printWarnings });
    }
    for (const operation of requiredImports_bvh_demo_ray_simple.operations) {
      Poly.registerOperation(operation, { printWarnings });
    }
    for (const jsFunction of requiredImports_bvh_demo_ray_simple.jsFunctions) {
  ...
  const sceneName = "bvh_demo_ray_simple";
...
export { Poly, loadSceneFromSceneData_bvh_demo_ray_simple };
